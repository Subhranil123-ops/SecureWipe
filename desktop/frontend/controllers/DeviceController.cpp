...
// ============================================================
// Request-bound target selection
// ============================================================
    ...
    const std::string &expectedSerialNumber)
{
    ...
    if (selectedTarget_->getSerialNumber() !=
    ...
    /*
     * The sanitization request owns the serial-number identity.
     * Bind the safety engine to that stable identity rather than
     * relying on a transient PhysicalDriveN path.
     */
...
// ============================================================
// Sanitization Method Detection
// ============================================================

SanitizationMethod
DeviceController::detectSelectedTargetMethod() const
{
    if (!selectedTarget_.has_value())
    {
...
// ============================================================
// Direct Sanitization Entry Point
// ============================================================
...
DeviceController::sanitizeSelectedTarget()
{
    if (!selectedTarget_.has_value())
    {
        ...
        result.errorMessage =
            "No sanitization target is selected.";
        return result;
    }
    ...
    if (!lastSafetyResult_.isOverallSafe)
    {
        ...
        result.status =
            SecureWipe::SanitizationStatus::FAILED;
        result.errorMessage =
            "Sanitization is blocked because the latest safety evaluation failed.";
        return result;
    }

    if (!sanitizationEngine_.canSanitize(
            *selectedTarget_,
            lastSafetyResult_))
    {
        SecureWipe::SanitizationResult result;
        result.status =
            SecureWipe::SanitizationStatus::FAILED;
        result.errorMessage =
            "The selected target cannot be sanitized.";
        return result;
    }

    return sanitizationEngine_.sanitize(
        *selectedTarget_,
        lastSafetyResult_);
}
