...
function ForensicCaseDetails() {
    ...
    const generateReport = async () => {
        try {
            ...
            const result =
                await generateForensicReport(caseId);
    ...
    const validationRate = useMemo(() => {
        ...
        return Math.round(
            (
                Number(item.validatedArtifacts || 0) /
                Number(item.recoveredArtifacts)
            ) * 100
        );
    }, [item]);
    ...
    const auditStats = useMemo(() => {
        ...
        const hashCount = auditTrail.filter(
            entry => Boolean(entry.eventHash)
        ).length;
        const chainedCount = auditTrail.filter(
            entry => Boolean(entry.previousEventHash)
        ).length;
    ...
    const tabs = [
        ...
        { id: "evidence", label: `Evidence (${artifacts.length})` },
        ...
        <div className="space-y-6">
            ...
            <div className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
                ...
                <div className="p-5">
                    ...
                    {activeTab === "timeline" && (
                        <TimelineTab history={item.history || []} />
                    )}
                    {activeTab === "audit" && (
                        <AuditTab
            ...
            <div className="grid grid-cols-1 gap-6 xl:grid-cols-3">
                <section className="rounded-lg border border-slate-200 bg-white shadow-sm">
                    ...
                    <div className="space-y-4 p-5">
                        ...
                        <Meta
                            ...
                            value={item.assignedEmployee?.name || "Not assigned"}
                        />
                        <Meta
                            label="Assigned workstation"
                            value={
                                item.assignedWorkstation?.name ||
                                item.assignedWorkstation?.workstationId ||
                                "Not assigned"
                            }
                        />
                ...
                <section className="rounded-lg border border-slate-200 bg-white shadow-sm xl:col-span-2">
                    ...
                    <div className="grid grid-cols-1 gap-5 p-5 sm:grid-cols-3">
                        <Meta label="Case ID" value={item.caseId} mono />
                        <Meta
                            label="Source identifier"
                            value={item.sourceIdentifier || "Not provided"}
                            mono
                        />
                        <Meta label="Read-only" value={item.readOnly ? "YES" : "NO"} />
                        <Meta
                            label="Employee"
                            value={item.assignedEmployee?.name || "Not assigned"}
                        />
                        <Meta
                            label="Workstation"
                            value={getCurrentWorkstationCode() || "Not assigned"}
                        />
                        <Meta label="Case created" value={formatDateTime(item.createdAt)} />
                    </div>
                </section>
            </div>
                ...
                <div className="space-y-5">
                    {center ? (
                        <>
                                ...
                                <p className="mt-1.5 text-xs text-slate-500">
                                    {employeeId
                                        ? availableWorkstations.length > 0
                                            ? `${availableWorkstations.length} active workstation${availableWorkstations.length === 1 ? "" : "s"} bound to ${selectedEmployee?.name || "selected employee"}.`
                                            : "This employee has no active workstation assigned."
                            ...
                            <div className="rounded-lg border border-amber-200 bg-amber-50 px-4 py-3">
                                ...
                                <p className="mt-1 text-xs leading-5 text-amber-700">
                                    The selected employee and workstation become part of the forensic case ownership record.
                                </p>
                            </div>
                        ...
                        <p className="text-sm text-slate-500">
                            Unable to load the workstation center assignment data.
                        </p>
                    )}
                </div>
            </ForensicModal>
        ...
        <div className="grid grid-cols-1 gap-6 xl:grid-cols-2">
            <section className="rounded-lg border border-slate-200">
                ...
                <div className="grid grid-cols-1 gap-5 p-5 sm:grid-cols-2">
                    <Meta
                        label="Source type"
                        value={
                            item.sourceType === "PHYSICAL_DEVICE"
                                ? "Physical device"
                                : "Forensic image"
                        }
                    />
                    ...
                    <Meta
                        label="Device type"
                        value={item.deviceType || "Not specified"}
                    />
                    <Meta
                        label="Capacity"
                    ...
                    <Meta
                        ...
                        value={item.assetIdentifier || "Not specified"}
                    />
                </div>
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-xs text-green-700">
                                Source modification is not part of the forensic workflow.
                            </p>
                        </div>
                    </div>
                </div>
            ...
            <section className="rounded-lg border border-slate-200">
                ...
                <div className="grid grid-cols-2 gap-px bg-slate-200 sm:grid-cols-4">
                    <Summary
                        label="Bytes scanned"
                        value={formatBytes(item.bytesScanned)}
                    />
                    <Summary
                        label="Total bytes"
                    ...
                    <Summary
                        label="Candidates"
                        value={item.candidatesFound || 0}
                    />
                    <Summary
                        label="Recovered bytes"
                        value={formatBytes(item.recoveredBytes)}
                    />
                </div>
            ...
            <section className="rounded-lg border border-slate-200 xl:col-span-2">
                ...
                <div className="grid grid-cols-1 gap-5 p-5 sm:grid-cols-3">
                    ...
                    <IntegrityCard
                        ...
                        value={item.rejectedArtifacts || 0}
...
function EvidenceTab({ artifacts, onInspect }) {
    if (!artifacts.length) {
        return (
            <EmptyPanel
        ...
        <div className="overflow-hidden rounded-lg border border-slate-200">
            <div className="border-b border-slate-200 bg-slate-50 px-5 py-4">
                ...
                <p className="mt-1 text-xs text-slate-500">
                    Recovered artifacts with validation, confidence and cryptographic integrity information.
                </p>
            </div>
                ...
                <table className="min-w-full">
                    ...
                    <tbody className="divide-y divide-slate-100">
                        {artifacts.map(artifact => (
                            <tr
                                key={artifact._id || artifact.artifactId}
                            ...
                            >
                                <td className="px-4 py-4">
                                    ...
                                    <p className="mt-1 max-w-[220px] truncate text-sm font-medium text-slate-900">
                                        {artifact.fileName || "Recovered artifact"}
                                    </p>
                                </td>
                        ...
                        <div className="min-w-0 rounded-lg border border-slate-200 bg-white p-4">
                            ...
                            {entry.changedBy && (
                                <p className="mt-2 text-[11px] text-slate-400">
                                    Changed by {typeof entry.changedBy === "object"
                                        ? entry.changedBy.name ||
                                          entry.changedBy.email ||
                                          "User"
                                        : String(entry.changedBy)}
                                </p>
                            )}
                        </div>
                    </div>
...
function AuditTab({ auditTrail, loading, stats, onRefresh }) {
    if (loading) {
        return (
        ...
        <div className="space-y-6">
            ...
            <div className="flex items-center justify-between">
                <div>
                    ...
                    <p className="mt-1 text-xs text-slate-500">
                        Each event points to the previous event hash.
                    </p>
                </div>
                                    ...
                                    >
                                        ...
                                        <td className="px-4 py-4">
                                            <div className="flex flex-wrap items-center gap-1.5">
                                                {event.fromStatus && (
                                                    <StatusPill value={event.fromStatus} />
                                                )}
                                                {event.toStatus && (
                                                    <>
                                                        <span className="text-slate-300">→</span>
...
function ReportTab({ item, onGenerate, loading }) {
        ...
        <div className="space-y-6">
            <section className="rounded-lg border border-green-200 bg-green-50 p-5">
                <div className="flex items-start justify-between gap-4">
                    <div>
                        ...
                        <p className="mt-1 text-xs text-green-700">
                            The report record contains the generated timestamp and report integrity hash.
                        </p>
                    </div>
                ...
                <div className="mt-5 grid grid-cols-1 gap-5 sm:grid-cols-2">
                    ...
                    />
                    <Meta label="Case status" value={item.status} />
                </div>
...
function IntegrityCard({ label, value, description }) {
    return (
